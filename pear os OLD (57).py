...
def load_icon(path, size=(50, 50)):
    ...
    try:
        # Note: Ensure you have an 'icons' folder with these filenames
        img = Image.open(path).convert("RGBA")
        img = img.resize(size, Image.Resampling.LANCZOS)
        return ImageTk.PhotoImage(img)
    except Exception as e:
        # Silent fallback to text-based icons if files are missing
        return None
...
def desktop():
    ...
    for i, (img_path, emoji, name, func) in enumerate(apps):
        ...
        # Try to load PNG, fallback to emoji
        tk_img = load_icon(f"icons/{img_path}")
        
        if tk_img:
            icon_images[name] = tk_img # Store reference
...
def open_music(e=None):
    ...
    if t: canvas.create_text(650, 350, text='Now Syncing: Aero Dreams.mp3', font=('Segoe UI', 18), tags=t)
...
def open_snake(e=None):
    ...
    root.bind('<w>', lambda e: direction != 'Down' and (set_dir := 'Up')) # Simplified binding logic
