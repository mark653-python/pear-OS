...
def toggle_fullscreen(e=None):
    ...
    # If exiting fullscreen, return to default size
    if not is_fullscreen:
        root.geometry('1400x850')

# Bind F11 for quick toggling
...
def boot():
    ...
    # Center coordinates relative to current window
    cx, cy = 700, 425
...
def login():
    ...
    for i in range(850):
        c = min(255, 150 + i // 5)
        canvas.create_line(0, i, 2000, i, fill=f'#66cc{c:02x}')
...
def desktop():
    ...
    for i in range(1200):
        c = min(255, 100 + i // 4)
        canvas.create_line(0, i, 2000, i, fill=f'#3399{c:02x}')
    ...
    for i in range(400):
        c = min(255, 120 + i // 2)
        canvas.create_line(0, 600+i, 2000, 600+i, fill=f'#{c:02x}dd{c:02x}')
...
def open_settings(e=None):
    ...
    if t: 
        ...
        fs_btn = canvas.create_rectangle(400, 350, 550, 390, fill=aero_blue_light, outline='white', tags=t)
        fs_txt = canvas.create_text(475, 370, text="Toggle Fullscreen", font=('Segoe UI', 10, 'bold'), tags=t)
        
        for item in [fs_btn, fs_txt]:
            canvas.tag_bind(item, '<Button-1>', toggle_fullscreen)
