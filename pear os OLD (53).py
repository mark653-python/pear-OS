...
def redraw_ui_overlay():
    ...
    apps = [
        ...
        ('🎵', 'Pear Tunes', open_music_player), # NEW APP
        ('🐍', 'Snake', open_snake), 
...
# ============================================
# MUSIC PLAYER (PEAR TUNES)
# ============================================
def open_music_player(e=None):
    t = create_window('Pear Tunes', '#1e1e1e')
    if not t: return
    
    current_song = tk.StringVar(value="No Song Loaded")
    ...
    def stop_music():
        ...
        canvas.itemconfig(song_label, text="Playback Stopped")
    ...
    stop_btn = canvas.create_rectangle(550, 480, 750, 520, fill='#ff4d4d', outline='white', tags=t)
    canvas.create_text(650, 500, text="Stop Playback", fill='white', font=('Segoe UI', 10, 'bold'), tags=t)
    canvas.tag_bind(stop_btn, '<Button-1>', lambda e: stop_music())

# ============================================
# WINDOW MANAGER & SYSTEM CORE
# ============================================
...
# [REMAINING APPS: SYSTEM, SETTINGS, SNAKE, CATCHER, SHOOTER, TERMINAL REMAIN INTEGRATED]
# (Included in final runtime but omitted here for brevity unless specific edits requested)
...
def open_terminal(e=None):
    ...
    def process(event):
        ...
        log.append(f"> {cmd}")
        if cmd == 'help': log.append("Commands: music, clear, exit")
...
# Game placeholders for the integrated build
