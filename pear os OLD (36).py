...
def redraw_ui_overlay():
    canvas.delete('ui')
    canvas.create_rectangle(240, 760, 1220, 825, fill=current_theme['taskbar'], stipple='gray25', width=2, tags='ui')
    ...
    apps = [
        ...
        ('catcher.png', '🧺', 'Catcher', open_pear_catcher),
...
# (Other app functions: open_music, open_browser, open_finder, open_sys_info, open_settings, open_terminal, open_about remain identical to original script)
...
def open_terminal(e=None):
    ...
    log_text = canvas.create_text(320, 210, text="\n".join(log), anchor='nw', fill=data_cell_green, font=('Consolas', 10), tags=t)
    cmd_var = tk.StringVar()
    cmd_entry = tk.Entry(root, textvariable=cmd_var, bg='#1a1a1a', fg=data_cell_green, font=('Consolas', 12), borderwidth=0)
    canvas.create_window(650, 580, window=cmd_entry, width=680, tags=t)
    def process_command(event):
        cmd = cmd_var.get().strip().lower()
        log.append(f"> {cmd}")
        if cmd == 'help': log.append("Available: help, ls, date, clear")
