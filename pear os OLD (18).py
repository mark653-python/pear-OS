...
def draw_default_gradient():
    """HD Sky & Ground Gradient with Glass-morphism"""
    for i in range(600):
...
def draw_background():
    ...
    # Push background to the very bottom layer
    canvas.tag_lower('bg')
...
def redraw_ui_overlay():
    ...
    # Bring UI layer to the very top layer
    canvas.tag_raise('ui')
...
def start_drag(event, tag):
    ...
    drag_data["x"] = event.x
    drag_data["y"] = event.y
    canvas.tag_raise(tag)      # Bring window to front
    canvas.tag_raise('ui')     # Keep taskbar on top
...
def create_window(title, color='#ffffff'):
    ...
    canvas.create_rectangle(300, 150, 1000, 190, fill='#44ee44', outline=data_cell_green, stipple='gray75', tags=(tag, title_tag))
    ...
    # Bind Drag Events (Disable for games due to absolute coordinate logic)
    if 'Snake' not in title and 'Apple' not in title:
        canvas.tag_bind(title_tag, '<ButtonPress-1>', lambda e, t=tag: start_drag(e, t))
        canvas.tag_bind(title_tag, '<B1-Motion>', drag)
        canvas.tag_bind(title_tag, '<ButtonRelease-1>', stop_drag)
...
def boot():
    ...
    canvas.create_text(700, 500, text='GAIASPHERE HD 2026', fill='white', font=('Segoe UI', 14, 'bold'), tags='boot')
