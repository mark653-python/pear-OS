...
# ============================================
# PEAR OS ULTIMATE GAIASPHERE - FINAL STABLE
# ============================================
...
def login():
    ...
    btn = canvas.create_rectangle(600, 550, 800, 610,
                                  fill='white', outline='white')
    txt = canvas.create_text(700, 580, text='Establish Connection',
                             font=('Segoe UI', 16), fill='#333')
...
def desktop():
    ...
    # GaiaSphere Central Orb
    canvas.create_oval(300, 100, 1100, 700,
                       fill='#33ccff',
    ...
    for i in range(250):
        c = min(255, 120 + i // 2)
        canvas.create_line(0, 600+i, 1400, 600+i,
                           fill=f'#{c:02x}dd{c:02x}')

    # Glass Dock
    canvas.create_rectangle(280, 760, 1180, 825,
                            fill='white',
                            outline='white',
                            stipple='gray50',
                            width=2)
    ...
    for i, (icon, name, func) in enumerate(apps):
        ...
        canvas.create_text(x, 835, text=name, fill='white',
                           font=('Segoe UI', 9))
...
def create_window(title, color='#ffffff'):
    ...
    # Shadow
    canvas.create_rectangle(305, 155, 1005, 605,
                            fill='black', stipple='gray25',
                            outline='', tags=tag)
    # Body
    canvas.create_rectangle(300, 150, 1000, 600,
                            fill=color, outline='white',
                            width=2, tags=tag)
    # Title Bar
    canvas.create_rectangle(300, 150, 1000, 190,
                            fill='#44ee44',
                            outline=data_cell_green,
                            stipple='gray75',
                            tags=tag)

    canvas.create_text(650, 170, text=title,
                       fill='white',
                       font=('Segoe UI', 14, 'bold'),
                       tags=tag)

    # Close Button
    btn = canvas.create_oval(310, 160, 330, 180,
                             fill='#ff4d4d',
                             outline='white',
                             tags=tag)

    canvas.tag_bind(btn, '<Button-1>',
                    lambda e: close_window(tag, title))
    return tag
...
def open_browser(e=None):
    ...
    if t:
        canvas.create_text(650, 250, text='://gaiasphere.feed',
                           fill=grass_green,
                           font=('Courier New', 14), tags=t)
        canvas.create_text(650, 400,
                           text='Welcome To The Glossy Web\n🌍 🫧 🐬',
                           font=('Segoe UI', 30),
...
def open_music(e=None):
    ...
    if t:
        canvas.create_text(650, 350,
                           text='Now Syncing: Aero Dreams.mp3',
                           font=('Segoe UI', 18), tags=t)
...
def open_settings(e=None):
    ...
    if t:
        canvas.create_text(400, 250,
                           text='Display: High Gloss\nNetwork: Connected\nUpdate: v1.0 Installed',
                           anchor='w',
                           font=('Segoe UI', 14), tags=t)
...
def open_terminal(e=None):
    ...
    if t:
        ...
        canvas.create_text(320, 210,
                           text=log,
                           anchor='nw',
                           fill=data_cell_green,
                           font=('Consolas', 12), tags=t)
...
def open_snake(e=None):
    ...
    def move():
        ...
        canvas.delete('snake')
        for s in snake:
            canvas.create_rectangle(s[0], s[1], s[0]+12, s[1]+12,
                                    fill=data_cell_green,
                                    tags=(tag, 'snake'))

        canvas.create_oval(food[0], food[1], food[0]+12, food[1]+12,
                           fill='#ff4d4d',
                           tags=(tag, 'snake'))
