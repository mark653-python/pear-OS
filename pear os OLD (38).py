...
def open_system(e=None):
    ...
    ram_label = canvas.create_text(330, 280, text="RAM Load: 0%", font=('Consolas', 11), anchor='w', tags=t)
    def update_stats():
        if not canvas.find_withtag(t): return
        cpu, ram = random.randint(5, 45), random.randint(20, 60)
    ...
    update_stats()
    btn = canvas.create_rectangle(330, 350, 500, 390, fill='#ff9966', outline='white', tags=t)
    canvas.create_text(415, 370, text="Purge Temp", fill='white', font=('Segoe UI', 10, 'bold'), tags=t)
    canvas.tag_bind(btn, '<Button-1>', lambda e: [messagebox.showinfo("System", "Cache Purged"), shake_window(5)])
