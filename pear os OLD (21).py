...
def play_dance_sound():
    try:
        # Note: Ensure random_dancing.mp3 is in the script directory
...
def start_random_dancing(count=20):
    if count > 0:
        ...
        # Add a little screen shake for the party effect
        shake_window(2)
...
def redraw_ui_overlay():
    ...
    # Random Dancing Button (Physical Desktop Element)
...
# ============================================
# WINDOW MANAGER & APPS (Logic remains same)
# ============================================
...
def open_icarly(e=None):
    webbrowser.open("https://www.google.com/search?q=iCarly+Official+Website")
...
def open_finder(e=None, path='.'):
    ...
    except Exception as err: files = [f"Error: {err}"]
    ...
    for i, item in enumerate(files[:12]):
        ...
        def handle_click(p=full_path, folder=is_dir, tag=t, win_title=title):
            if folder:
                ...
                try: os.startfile(p) if hasattr(os, 'startfile') else webbrowser.open(p)
...
def open_terminal(e=None):
    ...
    def process_command(event):
        ...
        if cmd == 'help': log.append("Available: help, ls, date, clear, i-ready, random-dancing, whoami, shake, exit")
        elif cmd == 'ls': log.append("Applications/  Documents/  Games/  System/  secret_pear_recipe.txt")
        elif cmd == 'date': log.append(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        elif cmd == 'whoami': log.append("TEST_USER_ALPHA")
        ...
        else: log.append(f"Command not found: {cmd}")
...
def open_about(e=None):
    ...
    if t: 
        ...
        canvas.create_text(650, 420, text='Built With Python + Tkinter by mark chang', fill='#44dd44', font=('Segoe UI', 14), tags=t)
